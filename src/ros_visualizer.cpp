...
namespace rpo
{
    ...
    void ROSVisualizer::showResult(RadiationPlan& radiation_plan)
    {
        ...
        {
            setModelColor(exposure_maps[0], true);
        }
    }


    void ROSVisualizer::setModelColor(const ExposureMap& dose_map, bool viridis)
    {
        for (const auto& element : dose_map)
        {
            const OcTreeKey key = element.first;

            const double exposure = element.second;
                    ...
                    else
                    {
                        if (viridis)
                        {
                            ...
                            node->setColor(
                                ...
                                static_cast<int>(255 * G[scale]),
                                static_cast<int>(255 * B[scale]));
                        }
                        else
                        {
                            node->setColor(
                                static_cast<int>(255 * R[0]),
                                static_cast<int>(255 * G[0]),
                                static_cast<int>(255 * B[0]));
                        }
                    }
                }
            }
